...
def ensure_configs(config: dict, required_configs: list) -> None:
    """
    Aborts application start up if there is any empty config values. It should
    be called at the application startup.
    ...
    if missing_configs:
        _logger.critical(
            f'Aborting process due to missing configs: {missing_configs}'
        )
        abort()
...
def inject_dependencies(
    ...
    Args:
        dependents (dict): Key can be any string which uniquely identifies
                           the dependant and the value must be any callable.
        providers (dict): Key should be name of the dependency and value should
                          be the actual value which `dependents` depend upon.
    ...
    for key, dependent in dependents.items():
        ...
        rescued_dependents[key] = \
            lambda *args, injections=injections, value=dependent: value(
                *args, **injections
            )
...
def register_blueprints(flask_app: Flask, module_paths: list) -> None:
    ...
    Args:
        ...
        route_modules (list): List of tuples/lists containing module path and
                              name of blueprint attribute.
    """
...
def register_http_error_handlers(flask_app: Flask) -> None:
    ...
    flask_app.register_error_handler(
        405,
        ErrorHandler.method_not_allowed_handler
    )
